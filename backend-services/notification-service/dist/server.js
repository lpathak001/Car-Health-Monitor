...
const logger = winston_1.default.createLogger({
    ...
    transports: [
        ...
        new winston_1.default.transports.File({ filename: 'notification.log' })
    ]
});
// Middleware
...
app.post('/api/notifications/send-alert', async (req, res) => {
    try {
        ...
        // Get user details
        const userResult = await pool.query('SELECT email, phone FROM users WHERE id = $1', [user_id]);
        ...
        if (alertChannels.includes('sms') && user.phone) {
            try {
                await axios_1.default.post('https://api.twilio.com/2010-04-01/Accounts/YOUR_ACCOUNT_SID/Messages.json', {
        ...
        if (alertChannels.includes('slack')) {
            try {
                ...
                if (slackWebhook) {
                    await axios_1.default.post(slackWebhook, {
        ...
        await pool.query(`INSERT INTO alerts (id, vehicle_id, user_id, alert_type, title, message, channels, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, [alertId, vehicle_id, user_id, alert_type, title, message, JSON.stringify(alertChannels)]);
        res.json({
            alert_id: alertId,
...
// 2. Get Alert History
app.get('/api/notifications/alerts/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
        const { limit = 50, offset = 0 } = req.query;
        const result = await pool.query(`SELECT id, vehicle_id, alert_type, title, message, created_at, read
       FROM alerts
       WHERE user_id = $1
       ORDER BY created_at DESC
       LIMIT $2 OFFSET $3`, [userId, limit, offset]);
        const countResult = await pool.query('SELECT COUNT(*) as total FROM alerts WHERE user_id = $1', [userId]);
        res.json({
            alerts: result.rows,
...
app.put('/api/notifications/alerts/:alertId/read', async (req, res) => {
    try {
        const { alertId } = req.params;
        await pool.query('UPDATE alerts SET read = true WHERE id = $1', [alertId]);
...
app.get('/api/notifications/unread-count/:userId', async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT COUNT(*) as count FROM alerts WHERE user_id = $1 AND read = false', [userId]);
...
app.post('/api/notifications/preferences/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
        const { email_enabled, sms_enabled, push_enabled, slack_enabled, alert_threshold } = req.body;
        await pool.query(`INSERT INTO notification_preferences (user_id, email_enabled, sms_enabled, push_enabled, slack_enabled, alert_threshold)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (user_id) DO UPDATE SET
       email_enabled = $2, sms_enabled = $3, push_enabled = $4, slack_enabled = $5, alert_threshold = $6`, [userId, email_enabled, sms_enabled, push_enabled, slack_enabled, alert_threshold]);
        // Clear cache
...
app.get('/api/notifications/statistics/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
       ...
       GROUP BY alert_type`, [userId, days]);
        const totalResult = await pool.query(`SELECT COUNT(*) as total, COUNT(CASE WHEN read = true THEN 1 END) as read_total
       FROM alerts
       WHERE user_id = $1 AND created_at > NOW() - INTERVAL '1 day' * $2`, [userId, days]);
        res.json({
            period_days: days,
            by_type: result.rows,
...
app.post('/api/notifications/bulk-send', async (req, res) => {
    try {
        ...
        for (const vehicleId of vehicle_ids) {
            try {
                const userResult = await pool.query('SELECT user_id FROM vehicles WHERE id = $1', [vehicleId]);
                if (userResult.rows.length > 0) {
                    const userId = userResult.rows[0].user_id;
                    // Send alert (simplified - would call send-alert endpoint)
                    const alertId = require('uuid').v4();
                    await pool.query(`INSERT INTO alerts (id, vehicle_id, user_id, alert_type, title, message, channels, created_at)
             VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, [alertId, vehicleId, userId, alert_type, title, message, JSON.stringify(channels || ['email'])]);
