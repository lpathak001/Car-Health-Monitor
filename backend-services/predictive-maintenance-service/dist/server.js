...
const logger = winston_1.default.createLogger({
    ...
    transports: [
        ...
        new winston_1.default.transports.File({ filename: 'predictive-maintenance.log' })
    ]
});
// Middleware
...
function predictComponentFailure(anomalyCount, avgValue, normalRange) {
...
app.get('/api/predictive/component-failures/:vehicleId', async (req, res) => {
    try {
...
app.get('/api/predictive/maintenance-schedule/:vehicleId', async (req, res) => {
    try {
        const { vehicleId } = req.params;
        const query = `
      SELECT 
...
app.get('/api/predictive/rul/:vehicleId', async (req, res) => {
    try {
        const { vehicleId } = req.params;
        const query = `
      SELECT 
    ...
    `;
        ...
        const degradationScore = ((anomalyRate * 0.4) +
...
app.get('/api/predictive/parts-inventory', async (req, res) => {
    try {
        const query = `
      SELECT 
        anomaly_type,
